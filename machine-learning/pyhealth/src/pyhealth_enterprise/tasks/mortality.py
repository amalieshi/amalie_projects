"""
In-hospital mortality prediction task setup.
...
def setup_mortality_task(
    dataset: Any,
    batch_size: int = 32,
    split: tuple[float, float, float] = (0.8, 0.1, 0.1),
) -> Tuple[DataLoader, DataLoader, DataLoader]:
    from pyhealth.datasets import get_dataloader, split_by_patient  # type: ignore[import]
    from pyhealth.tasks import mortality_prediction_mimic3_fn  # type: ignore[import]

    task_dataset = dataset.set_task(mortality_prediction_mimic3_fn)

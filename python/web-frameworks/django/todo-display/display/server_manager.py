...
class FastAPIServerManager:
    ...
    def __init__(self):
        ...
        # Handle Render deployment port configuration
        if os.environ.get("RENDER"):
            # On Render, use Django port + 1000 to avoid conflicts
            django_port = int(os.environ.get("PORT", 10000))
            self.port = django_port + 1000  # e.g., Django:10000 -> FastAPI:11000
            self.host = "0.0.0.0"
        else:
            # Local development
            self.port = int(os.environ.get("FASTAPI_PORT", 8000))
            self.host = "127.0.0.1"
    ...
    def test_port_connectivity(self, host, port, timeout=3):
        ...
        try:
            ...
            logger.info(f"Port connectivity test failed: {e}")
            return False

    def is_server_running(self):
        """Check if the FastAPI server is responding"""
        # Try multiple health check approaches for better reliability

        # Primary check: use localhost for health checks even when binding to 0.0.0.0
        health_check_host = "127.0.0.1" if self.host == "0.0.0.0" else self.host
        health_check_url = f"http://{health_check_host}:{self.port}/"
        logger.info(f"Checking server health at: {health_check_url}")

        # First test basic port connectivity
        if not self.test_port_connectivity(health_check_host, self.port):
            logger.info(f"Port {self.port} not connectable on {health_check_host}")
            return False

        logger.info(f"Port {self.port} is connectable, testing HTTP response...")
        ...
        try:
            ...
            logger.info(f"Server health check connection error: {e}")

            # Fallback: try with the original host if we changed it
            if health_check_host != self.host and self.host != "0.0.0.0":
                fallback_url = f"http://{self.host}:{self.port}/"
                logger.info(f"Trying fallback health check at: {fallback_url}")
                try:
                    response = requests.get(fallback_url, timeout=3)
                    logger.info(f"Fallback health check response: {response.status_code}")
                    return response.status_code == 200
                except requests.RequestException as fallback_e:
                    logger.info(f"Fallback health check also failed: {fallback_e}")
    ...
    def start_server_if_needed(self):
        ...
        if not self.fastapi_path.exists():
            ...
            return {
                "success": False,
                "message": f"FastAPI project path does not exist: {self.fastapi_path}",
            }
        ...
        try:
            ...
            logger.info(f"Target host:port: {self.host}:{self.port}")

            # Check if uvicorn is available before starting
            try:
                import uvicorn

                logger.info(f"uvicorn module found at: {uvicorn.__file__}")
            except ImportError as e:
                logger.error(f"uvicorn module not found: {e}")
                return {"success": False, "message": "uvicorn module not available"}
            ...
            while time.time() - start_time < max_wait:
                if process.poll() is not None:
                    ...
                    logger.error(
                        f"FastAPI server process terminated early. Return code: {process.returncode}"
                    )
                    logger.error(f"stdout: {stdout}")
                    logger.error(f"stderr: {stderr}")
                    return {"success": False, "message": f"Server process failed: {stderr}"}

                if self.is_server_running():
                    logger.info(f"FastAPI server started successfully on {self.host}:{self.port}")
                    return {
                        "success": True,
                        "message": f"Server started successfully on {self.host}:{self.port}",
                        "pid": process.pid,
                    }

                # Log current status every few attempts
                current_wait = time.time() - start_time
                if int(current_wait) % 5 == 0 and int(current_wait) > 0:
                    logger.info(f"Still waiting for server... ({int(current_wait)}s elapsed)")
            ...
            if process.poll() is None:
                ...
                logger.warning(f"Server started but not responding within {max_wait}s")

                # Try to get some output without waiting too long
                try:
                    stdout, stderr = process.communicate(timeout=2)
                    logger.warning(f"Process output - stdout: {stdout}")
                    logger.warning(f"Process output - stderr: {stderr}")
                except subprocess.TimeoutExpired:
                    logger.warning("Could not get process output (still running)")
                    process.kill()
                    stdout, stderr = process.communicate()
                    logger.warning(f"After kill - stdout: {stdout}")
                    logger.warning(f"After kill - stderr: {stderr}")

                return {
                    "success": False,
                    "message": "Server started but not responding to health checks",
                }
            else:
                stdout, stderr = process.communicate()
                logger.error(f"FastAPI server failed to start. stdout: {stdout}, stderr: {stderr}")
...
class DjangoPortManager:
    ...
    def ensure_port_available(self):
        ...
        logger.warning(
            f"Port {self.port} is in use by {len(processes)} process(es). Clearing conflicts..."
        )

        killed = self.kill_conflicting_processes()
        if killed:
            logger.info(
                f"Successfully killed {len(killed)} conflicting processes on port {self.port}"
            )
            time.sleep(2)  # Wait for port to be released
            ...
            if not remaining:
                return {
                    "success": True,
                    "message": f"Port {self.port} is now available after clearing conflicts",
                }
            else:
                logger.warning(
                    f"Port {self.port} still has {len(remaining)} processes after cleanup"
                )
            ...
            return {
                "success": False,
                "message": f"Port {self.port} conflicts could not be resolved",
            }

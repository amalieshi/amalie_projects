...
class FastAPIServerManager:
    ...
    def stop_server(self):
        ...
        try:
            ...
            return {"success": True, "message": f"Server stopped successfully (PID: {pid})"}
            
        except psutil.TimeoutExpired:
            try:
                process.kill()  # Force kill if graceful shutdown fails
                self._update_server_status(False, None)
                return {"success": True, "message": f"Server force-stopped (PID: {pid})"}
            except Exception as e:
                return {"success": False, "message": f"Error force-stopping server: {str(e)}"}
        except Exception as e:
            return {"success": False, "message": f"Error stopping server: {str(e)}"}
            
    def _update_server_status(self, is_running, pid):
        """Update server status in database"""
        status, created = ServerStatus.objects.get_or_create(defaults={
...
class APIClient:
    ...
    def _log_request(self, method, endpoint, request_data, response, execution_time):
        ...
        try:
            ...
            response_data = response.json() if response and hasattr(response, 'json') else None
    ...
    def update_todo(self, todo_id, title=None, description=None, completed=None):
        ...
        if completed is not None:
            ...
            # Debug logging
            print(f"DEBUG: Updating todo {todo_id} with completed={data['completed']} (type: {type(data['completed'])})")

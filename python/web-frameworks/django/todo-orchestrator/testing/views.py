...
class TodoManagementView(View):
    ...
    def get(self, request):
        ...
        # Debug logging
        print(
            f"DEBUG GET: Active todos count: {len(active_todos_result.get('data', [])) if active_todos_result.get('success') else 'FAILED'}"
        )
        print(
            f"DEBUG GET: Completed todos count: {len(completed_todos_result.get('data', [])) if completed_todos_result.get('success') else 'FAILED'}"
        )
        ...
        context = {
            ...
            "all_todos": (
                all_todos_result.get("data", [])
                if all_todos_result.get("success")
                else []
            ),
            "active_todos": (
                active_todos_result.get("data", [])
                if active_todos_result.get("success")
                else []
            ),
            "completed_todos": (
                completed_todos_result.get("data", [])
                if completed_todos_result.get("success")
                else []
            ),
    ...
    def post(self, request):
        ...
        if action == "delete":
            ...
            if result["success"]:
                messages.success(
                    request,
                    f"Todo deleted successfully! Status: {result['status_code']}",
                )
            else:
                messages.error(
                    request,
                    f"Error deleting todo: {result.get('error', 'Unknown error')}",
                )
            ...
            if current_todo["success"]:
                ...
                # Debug logging
                print(
                    f"DEBUG: Todo {todo_id} current status: {todo['completed']}, new status: {new_status}"
                )
                ...
                if result["success"]:
                    ...
                    messages.success(
                        request,
                        f"Todo '{todo['title']}' {status_text} successfully! Status: {result['status_code']}",
                    )
                    ...
                    messages.error(
                        request,
                        f"Error updating todo '{todo['title']}': {result.get('error', 'Unknown error')}",
                    )
                    print(f"DEBUG: Update failed - {result}")
            else:
                messages.error(
                    request,
                    f"Error getting todo details for ID {todo_id}: {current_todo.get('error', 'Unknown error')}",
                )
            ...
            result = client.update_todo(
                todo_id, title=title, description=description, completed=completed
            )

            if result["success"]:
                messages.success(
                    request,
                    f"Todo '{title}' updated successfully! Status: {result['status_code']}",
                )
            else:
                messages.error(
                    request,
                    f"Error updating todo: {result.get('error', 'Unknown error')}",
                )
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if result["success"]:
                messages.success(
                    request,
                    f"Todo created successfully! Status: {result['status_code']}",
                )
            else:
                messages.error(
                    request,
                    f"Error creating todo: {result.get('error', 'Unknown error')}",
                )
        else:
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if result["success"]:
                messages.success(
                    request,
                    f"Todo updated successfully! Status: {result['status_code']}",
                )
            else:
                messages.error(
                    request,
...
class RetrieveTodoView(View):
    ...
    def post(self, request):
        ...
        if result["success"]:
            messages.success(
                request, f"Data retrieved successfully! Status: {result['status_code']}"
            )
        else:
            messages.error(
                request,
                f"Error retrieving data: {result.get('error', 'Unknown error')}",
            )
...
class RawAPIRequestView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if result["success"]:
                messages.success(
                    request, f"Request completed! Status: {result['status_code']}"
                )
            else:
                messages.error(
                    request, f"Request failed: {result.get('error', 'Unknown error')}"
                )
        else:
            for field, errors in form.errors.items():
                messages.error(request, f"{field}: {', '.join(errors)}")
...
def health_check(request):
    ...
    return JsonResponse(
        {"status": "healthy", "service": "django-todo-orchestrator", "version": "1.0.0"}
    )
